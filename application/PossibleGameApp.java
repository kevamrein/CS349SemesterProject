...
/**
 * Main class for creating the application, handling user events, playing sounds and controlling the
 * game.
 * 
...
public class PossibleGameApp extends AbstractMultimediaApp implements KeyListener, MetronomeListener
{
  ...
  /**
   * Creates ENEMY_COUNT enemies, the player, and the score and lives displays.
   */
  ...
  /**
   * Removes all current components from the content pane and starts the game visualization.
   */
  ...
  /**
   * Removes all components from the content pane and displays the exit screen.
   */
  private void exit()
  {
    this.contentPane.removeAll();
  ...
  /**
   * Checks to see if the passed enemy hit the player on the side or on the top.
   * 
   * @param enemy
   *          Enemy
   * @return true if hit
